...
RemoveHeapTidsColumn(HnswVacuumState *vacuumstate, int col)
{
    ...
    BlockNumber blkno = HnswGetHeadBlockColumn(vacuumstate->index, col);

    HnswElement highestPoint = &vacuumstate->highestPoint;
    Relation index = vacuumstate->index;
    BufferAccessStrategy bas = vacuumstate->bas;

    /* entry point is column-specific */
    HnswElement entryPoint = HnswGetEntryPointColumn(index, col);
    ...
    while (BlockNumberIsValid(blkno))
    {
        ...
        for (offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno))
        {
            HnswElementTuple etup =
                (HnswElementTuple) PageGetItem(page, PageGetItemId(page, offno));
            int idx = 0;
            bool itemUpdated = false;
            ...
            if (!ItemPointerIsValid(&etup->heaptids[0]))
            {
                ...
                Assert(!found);
                     ...
                     !(entryPoint != NULL &&
                       blkno == entryPoint->blkno && offno == entryPoint->offno))
            {
                /* Keep track of highest non-entry point */
                highestPoint->blkno = blkno;
                highestPoint->offno = offno;
        ...
        /* Next page in THIS column's chain */
        blkno = HnswPageGetOpaque(page)->nextblkno;
...
/*
 * Repair graph entry point (column-specific)
 */
static void
RepairGraphEntryPointColumn(HnswVacuumState *vacuumstate, int col)
{
    Relation        index = vacuumstate->index;
    HnswSupport    *support = &vacuumstate->support;
    HnswElement     highestPoint = &vacuumstate->highestPoint;
    ...
    if (highestPoint != NULL)
    {
        ...
        /* Repair if needed */
        if (NeedsUpdated(vacuumstate, highestPoint))
            RepairGraphElement(vacuumstate, highestPoint, HnswGetEntryPointColumn(index, col));
    ...
    /* Get latest entry point (column-specific) */
...
/*
 * Repair graph for all elements (column-specific)
 */
static void
RepairGraphColumn(HnswVacuumState *vacuumstate, int col)
{
    Relation index = vacuumstate->index;
    BufferAccessStrategy bas = vacuumstate->bas;
    ...
    /* Repair entry point first (column-specific) */
    RepairGraphEntryPointColumn(vacuumstate, col);
    ...
    while (BlockNumberIsValid(blkno))
    {
        ...
        foreach (lc2, elements)
        {
            ...
            /* Refresh entry point for each element (column-specific) */
            entryPoint = HnswGetEntryPointColumn(index, col);
...
MarkDeletedColumn(HnswVacuumState *vacuumstate, int col)
{
    ...
    while (BlockNumberIsValid(blkno))
    {
        ...
        for (offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno))
        {
            ...
            /* Increment version (avoid incorrect reads for iterative scans) */
            etup->version++;
            if (etup->version > 15)
                etup->version = 1;
            ntup->version = etup->version;
    ...
    HnswUpdateMetaPageMulti(index,
                            ...
                            0,                  /* updateEntry: none */
                            NULL,               /* entryPoint */
                            insertPage,          /* per-column insertPage */
                            MAIN_FORKNUM,
                            false);
}
...
static void
InitVacuumStateColumn(HnswVacuumState * vacuumstate, IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state, int col)
{
	Relation index = info->index;
	...
	vacuumstate->efConstruction = HnswGetEfConstructionColumn(index, col);

	/* non-column-specific runtime resources */
	vacuumstate->bas = GetAccessStrategy(BAS_BULKREAD);
	vacuumstate->ntup = palloc0(HNSW_TUPLE_ALLOC_SIZE);
	vacuumstate->tmpCtx = AllocSetContextCreate(CurrentMemoryContext,
												"Hnsw vacuum temporary context",
	...
	/* column-specific support procs */
	HnswInitSupportColumn(&vacuumstate->support, index, col);

	/* Get m from metapage (column-specific) */
	HnswGetMetaPageInfoMulti(index, col, &vacuumstate->m, NULL);

	/* Create hash table (per column pass; ok to be separate) */
	vacuumstate->deleted = tidhash_create(CurrentMemoryContext, 256, NULL);
}
...
{
	Relation index = info->index;
	int nkeys = index->rd_index->indnkeyatts;  /* 仅 key 列 */
...
IndexBulkDeleteResult *
hnswbulkdelete_dispatch(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
						IndexBulkDeleteCallback callback, void *callback_state)
{
	...
	if (nkeys <= 1)
		...
		return hnswbulkdeletemulti(info, stats, callback, callback_state);
}
